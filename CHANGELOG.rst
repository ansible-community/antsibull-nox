...
- Antsibull-nox now allows to write output on succeeded and failed sessions for consumption by `ansibullbot <https://github.com/ansible-community/collection_bot/>`__ and as JUnit XML files for consumption by other tools. All built-in sessions support this, with varying degrees of useful information in the reports (https://github.com/ansible-community/antsibull-nox/pull/225).
- Explicitly set permissions to minimum for GitHub Actions and shared workflows (https://github.com/ansible-community/antsibull-nox/pull/236).
- Improve ``mypy`` output parsing; now support end position (https://github.com/ansible-community/antsibull-nox/pull/243).
- Improve ``ruff check`` output parsing; use name if code is not supplied (https://github.com/ansible-community/antsibull-nox/pull/243).
- Improve bot output for ``formatter`` session, and general output for ``ruff check --fix`` in CI (https://github.com/ansible-community/antsibull-nox/pull/241).
- Improve the GitHub Action by avoiding unnecessary nested groups (https://github.com/ansible-community/antsibull-nox/pull/247).
- Make output parsing more strict and stable (https://github.com/ansible-community/antsibull-nox/pull/243).
- Pin dependencies for GitHub Action and several important nox sessions (https://github.com/ansible-community/antsibull-nox/issues/246, https://github.com/ansible-community/antsibull-nox/pull/249).
- Run all tests in built-in sessions ``extra-checks``, ``docs-check``, ``license-check``, ``formatters``, ``codeqa``, and ``yamllint``, even if earlier tests in the same session fail (https://github.com/ansible-community/antsibull-nox/issues/228, https://github.com/ansible-community/antsibull-nox/pull/229).
